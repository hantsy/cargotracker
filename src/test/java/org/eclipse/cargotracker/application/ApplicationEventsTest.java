...
public class ApplicationEventsTest {
	...
	// verify(deliveredCargoConsumerSpy,
	// atLeastOnce()).onMessage(argumentCaptor.capture());
	//
	...
	public static WebArchive createDeployment() {
		...
		war.addClass(org.eclipse.cargotracker.infrastructure.messaging.JMSResources.class)
		...
		war
			// .addClass(ApplicationEventsTestDataGenerator.class)// use custom data
			// generator
	...
	public void testCargoWasHandled() throws InterruptedException {
		...
		var event = new HandlingEvent(cargo, LocalDateTime.now(), LocalDateTime.now(), HandlingEvent.Type.RECEIVE,
				SampleLocations.HONGKONG);
		applicationEvents.cargoWasHandled(event);

		await().atMost(Duration.ofMillis(1000))
			.untilAsserted(() -> assertThat(cargoInspectionService.getTrackingId()).isEqualTo(trackingId));
	}

	@Test
	public void testReceivedHandlingEventRegistrationAttempt() throws Exception {
		var trackingId = new TrackingId("AAA");
		var attempt = new HandlingEventRegistrationAttempt(LocalDateTime.now(), LocalDateTime.now(), trackingId,
				SampleVoyages.v100.getVoyageNumber(), HandlingEvent.Type.RECEIVE,
	...
	public static class HandlingEventServiceStub implements HandlingEventService {
		...
		@Override
		public void registerHandlingEvent(LocalDateTime completionTime, TrackingId trackingId,
