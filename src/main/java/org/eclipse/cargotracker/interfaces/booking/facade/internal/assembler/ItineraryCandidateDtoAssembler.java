...
public class ItineraryCandidateDtoAssembler {
	...
	protected LegDto toLegDto(Leg leg) {
		...
		return new LegDto(voyageNumber.number(), locationDtoAssembler.toDto(leg.getLoadLocation()),
				locationDtoAssembler.toDto(leg.getUnloadLocation()), leg.getLoadTime(), leg.getUnloadTime());
	}

	public Itinerary fromDto(RouteCandidateDto routeCandidate, VoyageRepository voyageRepository,
			LocationRepository locationRepository) {
		List<Leg> legs = routeCandidate.legs().stream().map(leg -> {
			VoyageNumber voyageNumber = new VoyageNumber(leg.voyageNumber());
			Voyage voyage = voyageRepository.find(voyageNumber);
			Location from = locationRepository.find(new UnLocode(leg.from().code()));
			Location to = locationRepository.find(new UnLocode(leg.to().code()));
			return new Leg(voyage, from, to, leg.loadTime(), leg.unloadTime());
		}).toList();
